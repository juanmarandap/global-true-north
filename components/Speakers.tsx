...
const SPEAKERS: Speaker[] = [
  ...
  { name: "Andre Wegner",           category: "Manufactura Digital",        photo: "/images/speakers/andre-wegner.jpg",      objectPosition: "center top",      bio: "Experto en cómo la tecnología está redefiniendo la manufactura. Desde 3D printing hasta automatización inteligente, ayuda a empresas a reinventar cómo producen y operan.",                                                                                                                                     bioEn: "Expert in how technology is redefining manufacturing. From 3D printing to intelligent automation, he helps companies reinvent how they produce and operate." },
...
function SpeakerAvatar({ name, photo, color, objectPosition, objectFit = "cover" }: { name: string; photo: string; color: string; objectPosition?: string; objectFit?: "cover" | "contain" }) {
...
function SpeakerRow({ speaker, index }: { speaker: Speaker; index: number }) {
  ...
  const photoBlock = (
    ...
    >
      <SpeakerAvatar name={speaker.name} photo={speaker.photo} color={color} objectPosition={speaker.objectPosition} objectFit={speaker.objectFit} />
    </motion.div>
  );
